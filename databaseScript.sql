...
-- Table `conushop`.`Item`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `conushop`.`Item` ;

CREATE TABLE IF NOT EXISTS `conushop`.`Item` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `Electronic_id` INT NULL,
  `serialNumber` VARCHAR(45) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Item_Electronic1_idx` (`Electronic_id` ASC),
  CONSTRAINT `fk_Item_Electronic1`
    FOREIGN KEY (`Electronic_id`)
    REFERENCES `conushop`.`Electronic` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
REPLACE INTO User (id, email, password, admin) values (1, 'admin@conushop.com', '$2y$10$CCdVyhydRyjluMY7/39VL.A1atziI7EAdRHhWyFkZyMKOfMlFl3GW', 1);
